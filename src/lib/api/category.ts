...
export const updateCategory = async (category_id: string, name: string): Promise<any> => {
	try {
		const data = { name }; // Simplify object assignment
		const record = await Base.getPocketbase().collection('category').update(category_id, data);
		return record;
	} catch (error) {
