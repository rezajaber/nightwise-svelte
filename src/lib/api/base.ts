...
export class BaseService {
	...
	async authenticateWithProvider(provider: string) {
		try {
			const authData = await this.pb.collection('user').authWithOAuth2({ provider: provider });
...
export const base = new BaseService(import.meta.env.VITE_POCKETBASE_URL as string);
