...
export const getPrio = async (): Promise<any[]> => {
	try {
		const records = await Base.getPocketbase().collection('prio').getFullList({ sort: '-created' });
		return records;
	} catch (error) {
...
export const updatePrio = async (prio_id: string, name: string): Promise<any> => {
	try {
		const data = { name };
		const record = await Base.getPocketbase().collection('prio').update(prio_id, data);
		return record;
	} catch (error) {
